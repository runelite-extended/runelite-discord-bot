...
function updateStatus () {
  fetch('https://api.github.com/repos/runelite-extended/runelite/tags', {
    headers: {
      Authorization: `token ${config.githubToken}`
    }
  }).then(res => res.json())
    .then(body => {
      return fetch(`https://session.runelitepl.us/count`)
    })
    .then(res => res.json())
...
client.on('ready', () => {
  ...
  client.setInterval(() => fetchContributors(client.guilds, 0), 300000)
})
